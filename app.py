...
def find_invite_code(data: dict, code: str):
    code_norm = (code or '').strip().upper()
    for c in data.get('invite_codes', []):
        if str(c.get('code', '')).upper() == code_norm:
            return c
    return code_norm and None


def invite_code_valid(entry: dict) -> tuple:
    ...
    if exp:
        try:
            ...
            pass
    max_uses = entry.get('max_uses', 1) or 0
    if max_uses and entry.get('uses', 0) >= max_uses:
        return False, 'exhausted'
    return True, 'ok'


def register_user_with_code(code: str, telegram_id: str, first_name: str = '') -> dict:
    """Validate an invite code and create a panel user bound to telegram_id.
    Returns {'status':'success','user_id':...} or {'error':...}.
    Thread-safe enough for the single-process bot: load -> mutate -> save."""
    data = load_data()
    tg_clean = str(telegram_id).lstrip('@')

    # Already registered?
    for u in data.get('users', []):
        if str(u.get('telegramId', '') or '').lstrip('@') == tg_clean:
            return {'error': 'already_registered', 'user_id': u['id']}

    entry = find_invite_code(data, code)
    ok, reason = invite_code_valid(entry)
    if not ok:
        return {'error': reason}

    base_username = f"tg{tg_clean}"
    ...
    new_user = {
        ...
        'telegramId': tg_clean,
        'email': None,
        'description': f"Self-registered via Telegram ({first_name})".strip(),
        'traffic_limit': int(entry.get('traffic_limit', 0) or 0),
        'traffic_reset_strategy': entry.get('traffic_reset_strategy', 'never'),
        'traffic_used': 0,
        'traffic_total': 0,
        'last_reset_at': datetime.now().isoformat(),
        'expiration_date': entry.get('user_expiration_date'),
        'enabled': True,
...
def create_user_connection(user_id: str, server_id: int, protocol: str, name: str) -> dict:
    """Create a real client on the server and persist a user_connection.
    Returns {'config':..., 'vpn_link':..., 'connection': {...}} or {'error':...}.
    Blocking (SSH) — bot must call via asyncio.to_thread."""
    data = load_data()
    ...
    if result.get('config'):
        ...
        out['vpn_link'] = generate_vpn_link(result['config'])
    return out
...
def create_invite_codes_for_bot(count: int = 1, max_uses: int = 1, note: str = '', created_by: str = 'bot') -> list:
    data = load_data()
    existing = {c.get('code') for c in data.get('invite_codes', [])}
    created = []
    for _ in range(max(1, min(count, 20))):
        ...
        existing.add(code)
        entry = {
            'code': code, 'note': note or '', 'role': 'user',
            'max_uses': max(0, max_uses), 'uses': 0, 'used_by': [],
...
def get_client_config_for_connection(conn: dict) -> Optional[str]:
    """Fetch config for an already-existing user_connection (used by 'My configs')."""
    data = load_data()
    servers = data.get('servers', [])
    sid = conn.get('server_id', 0)
    if sid >= len(servers):
    ...
    try:
        ...
        if proto == 'wireguard':
            return manager.get_client_config(conn['client_id'], server['host'])
        return manager.get_client_config(proto, conn['client_id'], server['host'], port)
    finally:
        ssh.disconnect()
...
async def startup():
    ...
    if tg_cfg.get('enabled') and tg_cfg.get('token'):
        ...
        tg_bot.launch_bot(tg_cfg['token'], bot_services())
...
async def api_telegram_toggle(request: Request):
    ...
    if tg_bot.is_running():
        ...
        tg_bot.launch_bot(token, bot_services())
        tg_cfg['enabled'] = True
        data['settings']['telegram'] = tg_cfg
        save_data(data)
...
@app.get('/api/invite-codes', tags=["Invite Codes"])
async def api_list_invite_codes(request: Request):
    if not _check_admin(request):
        return JSONResponse({'error': 'Forbidden'}, status_code=403)
    data = load_data()
    codes = []
...
@app.post('/api/invite-codes', tags=["Invite Codes"])
async def api_create_invite_code(request: Request, req: CreateInviteCodeRequest):
    if not _check_admin(request):
        return JSONResponse({'error': 'Forbidden'}, status_code=403)
    if req.role not in ('admin', 'support', 'user'):
        return JSONResponse({'error': 'Invalid role'}, status_code=400)
    ...
    async with DATA_LOCK:
        ...
        for _ in range(max(1, min(req.count, 100))):
            ...
            entry = {
                ...
                'created_at': datetime.now().isoformat(),
                'created_by': cur['username'] if cur else None,
            }
            data.setdefault('invite_codes', []).append(entry)
...
@app.delete('/api/invite-codes/{code}', tags=["Invite Codes"])
async def api_delete_invite_code(request: Request, code: str):
    if not _check_admin(request):
        return JSONResponse({'error': 'Forbidden'}, status_code=403)
    async with DATA_LOCK:
        data = load_data()
        before = len(data.get('invite_codes', []))
